# 这个是没有组件丶没有生命周期丶没有除了（v-if丶v-for丶双括号表达式丶watch丶computed）以外的所有属性，甚至包括原生js属性都不支持

这个项目的目的是让那些只是打算大致了解模板编译流程，数据响应式原理，计算属性，侦听器，diff算法，virtual DOM思想，next-tick原理的人，简化了代码结构，更容易理解，如果条件允许的话上手写一些代码相信会理解的更深刻的。
