...
export function generate(ast,options){                      //直接的generate方法
    const state = new CodegenState(options)             //目前还是不太懂这个是干吗用的
    const code = ast ? genElement(ast, state) : '_c("div")'             //没有ast就生成一个空节点
...
export function genElement (el, state){
    ...
    } else if (el.tag === 'template' && !state.pre){               //template
